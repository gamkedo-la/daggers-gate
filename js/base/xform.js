...
class XForm {
    ...
    constructor(spec={}) {
        ...
        this._origx = Util.objKeyValue(spec, "origx", .5);
        this._origy = Util.objKeyValue(spec, "origy", .5);
    ...
    apply(ctx) {
        ...
        if (this.dbg && (dox || doy)) console.log("apply dox: " + dox + " doy: " + doy);
        if (this.angle) ctx.rotate(this.angle);
        if (this.dbg && this.angle) console.log("apply rot: " + this.angle);
        if (this.scalex !== 1|| this.scaley !== 1) ctx.scale(this.scalex, this.scaley);
        if (this.dbg && (this.scalex !== 1|| this.scaley !== 1)) console.log("apply scale: " + this.scalex + "," + this.scaley);
        if (this.dx || this.dy) ctx.translate(this.dx, this.dy);
        if (this.dbg && (this.dx || this.dy)) console.log("apply dx: " + this.dx + " dy: " + this.dy);
    }
    ...
    revert(ctx) {
        ...
        let doy = this.doy;
        if (dox || doy) ctx.translate(-dox, -doy);
        if (this.parent) this.parent.revert(ctx);
    }
    ...
    getWorld(localPos) {
        ...
        // apply local transforms
        //if (this.dx || this.dy) console.log("sub: " + this.dx + "," + this.dy);
        if (this.dx || this.dy) worldPos.add(this.dx, this.dy);
        if (this.scalex !== 1|| this.scaley !== 1) worldPos.mult(this.scalex, this.scaley);
        //if (this.scalex !== 1|| this.scaley !== 1) console.log("scale: " + this.scalex + "," + this.scaley);
        if (this.angle) worldPos.rotate(this.angle, true);
        //if (this.angle) console.log("rotate: " + this.angle);
        // perform world->local translation
        worldPos.add(this.dox, this.doy);
        //if (this.dox || this.doy) console.log("sub do: " + this.dox + "," + this.doy);
        // apply parent transform (if any)
